...
cout<<endl;
    ...
    for (int i = 1; i< size ; i++)
	{
		...
		double  denom = (p2x-p1x)^2 + (p2y-p1y)^2;
		ushort d = abs((p2x-p1x)*(p1y-p0y)-(p1x-p0x)*(p2y-p1y))/sqrt(denom);
        if(d<=epsilon)
		{
...
//cout<<endl;
    ...
    for (int i = 0; i< size ; i++)
	{
        int p0x = XY[i].x;
        int p0y = XY[i].y;
		int d = abs((p2x-p1x)*(p1y-p0y)-(p1x-p0x)*(p2y-p1y))/sqrt((double)(p2x-p1x)*(p2x-p1x) + (p2y-p1y)*(p2y-p1y));
        if(d<=epsilon)
		{
    ...
    if(Inliers_size>=threshold)
	{
		...
		angle = atan2((double)(p2y - p1y),(double)( p2x - p1x)) * 180 / PI;
		lineLength = sqrt(pow((double)p2x - p1x, 2) + pow((double)p2y - p1y, 2));
