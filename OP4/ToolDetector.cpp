		...
		for (int j = 0; j < (greySearchWin.rows - greyBlock.rows) / step; j+=step){
			...
			for (int k = 0; k < greyBlock.cols; k++) {
				for (int l = 0; l < greyBlock.rows; l++) {
					rms += pow(greyBlock.at<uchar>(l,k) - greySearchWin.at<uchar>(j + l,i + k), 2);
	...
	*/
	...
	for (;;)
	{
		...
		if (!tracking) {
			...
			if (P2[0] == 0 || mostLikelyCluster.size() < 20 || computeRMS(shaftClosestPoint, clusterCenterPoint) > 30)
			{
				Box = R;
				tracking = false;
			...
			searchWindow = Rect(max(estimatedRegion.x - rectMargin/2,0) , max(estimatedRegion.y - rectMargin/2,0) 
				               ,min(estimatedRegion.width + rectMargin,resizedFrame.cols)
							   ,min(estimatedRegion.height + rectMargin,resizedFrame.rows));
			//cout << "ROI : [" << estimatedRegion.x << "," << estimatedRegion.y<< "]"<<endl;
			//cout << "SW : [" << estimatedRegion.x - rectMargin / 2 << "," << estimatedRegion.y - rectMargin / 2 << "]" << endl;
			//vector<vector<Point>> features;
			//findContours(resizedFrame(estimatedRegion), features, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
			//drawContours(resizedFrame(estimatedRegion), features, 0, Scalar(255, 0, 255));
			double matchingError = 0.0;
			
			Point2d motionVec = motionEstimation(imgROI, resizedFrame(searchWindow), 1, matchingError);
			cout << "mV:" << motionVec.x << " , " << motionVec.y << endl;
			
			if (matchingError < 50){
				if (   estimatedRegion.x - rectMargin / 2 + motionVec.x < resizedFrame.cols// - rectMargin
					&& estimatedRegion.y - rectMargin / 2 + motionVec.y < resizedFrame.rows// - rectMargin
					&& estimatedRegion.x - rectMargin / 2 + motionVec.x > 0
					&& estimatedRegion.y - rectMargin / 2 + motionVec.y > 0
					) {
					//estimatedRegion.x += (motionVec.x - rectMargin/2);
					//estimatedRegion.y += (motionVec.y - rectMargin/2);
					estimatedRegion.x = searchWindow.x + motionVec.x;
					estimatedRegion.y = searchWindow.y + motionVec.y;
					
					Mat subtracted;
					subtract(resizedFrame(estimatedRegion), imgROI, subtracted);
					imshow("matched", subtracted);

					cout << "New ROI:" << estimatedRegion.x << " , " << estimatedRegion.y << endl;
					//imgROI = resizedFrame(estimatedRegion);
				}
				rectangle(resizedFrame, searchWindow, Scalar(0, 255, 0), 3);
				rectangle(resizedFrame, estimatedRegion, Scalar(0, 0, 255), 3);
				//imshow("matched", resizedFrame(estimatedRegion));
				//imshow("SW", resizedFrame(searchWindow));
			}
			else {
				cout << "matching error:" << matchingError << endl;
				tracking = false;
			}
