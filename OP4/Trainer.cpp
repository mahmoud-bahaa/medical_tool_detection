...
Mat Trainer::CreateTrainingData_integral(String video_name, String video_short_name,String file_name, int window_width, int window_height, int block_size, int stride, int cell_size, int bins, int resize_amount)
{
	...
	for (int i = 0;i<strs.size();i++)
	{
		...
		std::string groundTruthFile = groundTruthPath + to_string(strs[i]) + ".txt";
		...
		ofstream sampleOut(video_name+"\\..\\..\\..\\"+file_name+"\\..\\"+to_string(strs.at(i))+".txt");
		sampleOut << to_string(strs.at(i)) << ";w;2;" << cx << ";" << cy << ";" << cx << ";" << cy << ";-1;-1;-1;-1;whatever;"<<video_short_name;
		sampleOut.close();
